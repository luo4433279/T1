...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        //http.rememberMe().userDetailsService(userDetailService).key(key).useSecureCookie(false).alwaysRemember(false);
        ...
        http.formLogin().loginProcessingUrl("/login").loginPage("/admin/to-login").defaultSuccessUrl("/admin").successHandler(new AuthenticationSuccessHandler());
        //http.logout().logoutSuccessHandler(new LogoutSuccessHandler());
        http.logout().logoutUrl("/logout").logoutSuccessUrl("/to-login").invalidateHttpSession(true).logoutSuccessHandler(new LogoutSuccessHandler());
    ...
    private class MyAuthenticationEntryPoint implements AuthenticationEntryPoint {
        ...
        public void commence(HttpServletRequest request,
            ...
            if (isAjax(request)) {
            	...
            	if (StringUtils.equals(request.getRequestURI(), "/admin/")){
            		response.sendRedirect("/admin/to-login");
            	}else{
            		response.sendRedirect("/admin/to-login?error");
            	}
            }
